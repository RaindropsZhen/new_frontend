import { Form, Button } from "react-bootstrap"; // InputGroup removed
import React, { useState, useContext } from "react"; // Row removed
...
// import { uploadImage } from "../apis"; // uploadImage import removed
...
const PlaceForm = ({ onDone, place_length }) => {
  ...
  const [selectedDinnerTimeFinish, setselectedDinnerTimeFinish] =
    useState(82800); // Initialize with 11PM = 23*3600
  ...
  const onClick = async () => {
    ...
    if (image) {
      // Ensure image is not null/undefined before appending
    ...
    for (const categoryName of categoryNames) {
      await addCategory( // cat_json assignment removed
        {
          name: categoryName,
          place: place_json.id,
          languages: languages,
