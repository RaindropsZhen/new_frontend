...
import { addCategory, addMenuItems } from "../apis"; // uploadImage removed
...
const MenuItemForm = ({ place, onDone, item = {} }) => {
  ...
  // const allFieldsFilled = () => { // Removed unused function
  ...
  const onAddMenuItems = async () => {
    ...
    if (responseJson && responseJson.success) {
      ...
      toast(`成功创建菜品 ${responseJson.menu_item_name || name}`, {
        type: "success",
      }); // Fallback to name if menu_item_name is undefined
      setName("");
      setPrice("");
      setCategoryName("");
      setDescription("");
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Error loading selected languages from local storage:",
        error,
      );
    }
  }, []);
  ...
  const lunchTimeDisplay =
    typeof lunchStartTime === "number" && typeof lunchEndTime === "number"
      ? `(${formatTime(lunchStartTime)} - ${formatTime(lunchEndTime)})`
      : t("menuSettings.timeNotSet", "(未设置)");

  const dinnerTimeDisplay =
    typeof dinnerStartTime === "number" && typeof dinnerEndTime === "number"
    ...
    <Form>
      ...
      <Form.Group>
        ...
        <div className="d-flex align-items-center">
          ...
          >
            <option value="">请选择以下一个菜品</option>{" "}
            {/* Blank or default option */}
