...
import { updateMenuItem } from "../apis"; // uploadImage removed
...
const useEditMenuItemFormLogic = (initialItem = {}, place, onDoneCallback) => {
  ...
  const [price, setPrice] = useState(
    initialItem.price || t("editMenuItemForm.placeholder.enterPrice"),
  );
  const [image, setImage] = useState(initialItem.image);
  const [isAvailable, setIsAvailable] = useState(
    initialItem.is_available === undefined ? true : !!initialItem.is_available,
  );
  const [orderingTiming, setOrderingTiming] = useState(
    initialItem.ordering_timing,
  );
  ...
  const onUpdateMenuItem = async () => {
    ...
    if (isNaN(numericPrice) || numericPrice < 0) {
      toast.error(
        t(
          "menuSettings.toast.priceMustBePositive",
          "价格必须为大于或等于0的数字。",
        ),
      ); // Add this key to translation
      return;
    }
    ...
    if (json) {
      toast(
        t("editMenuItemForm.toast.updateSuccess", { itemName: json.name }),
        { type: "success" },
      );
      setCategory("");
