...
import {
  ...
  reprintOrder,
  fetchPlace,
  updateTableBlockedStatus,
} from "../apis"; // Removed updateTableNumberPeople
...
const Orders = () => {
  ...
  const onFetchOrders = useCallback(async () => { // Wrapped in useCallback
    const json = await fetchOrders(params.id, auth.token);
    if (json) {
      setOrders(json);
    }
  }, [params.id, auth.token]); // Added dependencies
  ...
  }, [onFetchOrders]); // Added onFetchOrders to dependencies
  ...
  // const sortedTableNumbers = Object.keys(groupedByTable).sort((a, b) => a - b); // Removed unused variable
              ...
              <Col key={table.id} xs={6} sm={4} md={3} lg={2} className="mb-4">
                ...
                <div className="d-flex flex-column gap-2">
                  <div className="d-flex align-items-stretch">
                    <Button
                      variant={
                        groupedByTable[table.table_number]
                          ? "outline-primary"
                          : "outline-secondary"
                      }
                      block
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          `/menu/0/${params.id}/${table.table_number}`,
                          "_blank",
                        )
                      }
                  ...
                  <ToggleButton
                    ...
                    onChange={async (e) => {
                      if (e.currentTarget) {
                        ...
                        if (table) {
                          await updateTableBlockedStatus(
                            table.id,
                            isChecked,
                            auth.token,
                          );
                          ...
                          toast.success(
                            isChecked
                              ? `桌号 ${table.table_number} 已封锁。`
                              : `桌号 ${table.table_number} 已解除封锁。`,
                          );
                        }
                      }
                    }}
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="xl">
        ...
        <Modal.Body>
          {Object.keys(selectedTableOrders).length > 0 ? (
            (() => {
              const filteredAndSortedOrders = Object.entries(
                selectedTableOrders,
              ).sort(([, ordersA], [, ordersB]) => {
                const timeA = ordersA[0]?.created_at
                  ? new Date(ordersA[0].created_at).getTime()
                  : 0;
                const timeB = ordersB[0]?.created_at
                  ...
                  <div key={dailyId}>
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <Button
                        variant="success"
                        onClick={() =>
                          handleUpdateOrderStatus(selectedTable, dailyId)
                        }
                        disabled={
