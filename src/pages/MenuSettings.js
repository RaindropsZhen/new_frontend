...
import { fetchPlace, addCategory, removeCategory, updateCategory, addMenuItems, removeMenuItem, updateMenuItem, reorderCategories, reorderMenuItems } from '../apis'; // Added reorderCategories, reorderMenuItems
      ...
      <Row>
        <Col md={4}>
          <Panel>
            ...
            <DragDropContext onDragStart={() => setIsDragging(true)} onDragEnd={handleOnCategoryDragEnd}>
                        ...
                        return (
                          <Draggable key={String(category.id)} draggableId={String(category.id)} index={index}>
                            {(providedDraggable, snapshot) => (
                              ...
                              >
                                <div {...providedDraggable.dragHandleProps} style={{ cursor: 'grab', marginRight: '10px', display: 'flex', alignItems: 'center' }}>
                                  <FiMenu size={18} />
                                </div>
        ...
        <Col md={8}>
          {selectedCategoryId && currentCategory ? (
            <Panel>
              ...
              <DragDropContext onDragStart={() => setIsDragging(true)} onDragEnd={(result) => handleOnMenuItemDragEnd(result, currentCategory.id)}>
                    ...
                    >
                      {currentCategory.menu_items?.length > 0 ? (
                        currentCategory.menu_items.map((item, index) => {
                          ...
                          return (
                            <Draggable key={String(item.id)} draggableId={`menuitem-${String(item.id)}`} index={index}>
                              {(providedItem, snapshotItem) => (
                                <StyledListItem 
                                  ...
                                  style={{
                                    ...
                                    backgroundColor: snapshotItem.isDragging ? '#d0eaff' : 'transparent',
                                  }}
                                >
                                  <div {...providedItem.dragHandleProps} style={{ cursor: 'grab', marginRight: '10px', display: 'flex', alignItems: 'center' }}>
                                    <FiMenu size={18} />
                                  </div>
                                  {imageUrl && (
                                    <img 
                                  ...
                                  <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    <span className="item-name">{itemDisplayLangName} - €{item.price ? item.price.toFixed(2) : '0.00'}</span> 
                                    {descriptionPreview && (
                        ...
                        !isDragging && <p className="text-center text-muted mt-3">{t('menuSettings.noMenuItemsYet')}</p>
                      )}
