...
import { FiEdit2, FiTrash2, FiMenu } from "react-icons/fi"; // Added FiMenu for drag handle
...
import { DragDropContext, Droppable, Draggable } from "react-beautiful-dnd"; // Added D&D imports
...
import {
  ...
  removeCategory,
  updateCategory,
  // addMenuItems, // Not directly used, MenuItemForm handles it
  removeMenuItem,
  // updateMenuItem, // Not directly used, EditMenuItemForm handles it
  reorderCategories,
  reorderMenuItems,
} from "../apis"; // Added reorderCategories, reorderMenuItems
...
const MenuSettings = () => {
  ...
  const [isDragging, setIsDragging] = useState(false); // Added for D&D state
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const result = await addCategory(
        { name: newCategoryName, place: params.id },
        auth.token,
      );
      if (result) {
        toast.success(
          t("menuSettings.toast.categoryAddedSuccess", {
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      ...
      if (result) {
        toast.success(
          t("menuSettings.toast.categoryUpdatedSuccess", {
  ...
  const handleDeleteCategory = async (categoryId, categoryName) => {
    ...
    const catNameToConfirm =
      categoryName || t("menuSettings.thisCategory", "this category");
    if (
      window.confirm(
        t("menuSettings.confirm.deleteCategory", {
          categoryName: catNameToConfirm,
        }),
      )
    ) {
      try {
        ...
        if (success) {
          toast.success(
            t("menuSettings.toast.categoryDeletedSuccess", {
              categoryName: catNameToConfirm,
            }),
          );
          onFetchPlace();
  ...
  const handleDeleteMenuItem = async (itemId, itemName) => {
    const itemNameToConfirm =
      itemName || t("menuSettings.thisMenuItem", "this menu item");
    if (
      window.confirm(
        t("menuSettings.confirm.deleteMenuItem", {
          itemName: itemNameToConfirm,
        }),
      )
    ) {
      try {
        ...
        if (success) {
          toast.success(
            t("menuSettings.toast.menuItemDeletedSuccess", {
              itemName: itemNameToConfirm,
            }),
          );
          onFetchPlace();
  ...
  const currentCategory = place.categories?.find(
    (c) => c.id === selectedCategoryId,
  );

  const handleOnCategoryDragEnd = async (result) => {
    setIsDragging(false);
    if (
      !result.destination ||
    ...
    try {
      ...
      toast.success(t("menuSettings.toast.categoriesReorderedSuccess"));
      onFetchPlace();
    } catch (error) {
      console.error("Failed to reorder categories:", error);
      toast.error(t("menuSettings.toast.categoriesReorderFailedError"));
      onFetchPlace();
    }
  };
  ...
  const handleOnMenuItemDragEnd = async (result, categoryId) => {
    ...
    if (!result.destination || !currentCategory || !currentCategory.menu_items)
      return;
    ...
    try {
      ...
      toast.success(t("menuSettings.toast.menuItemsReorderedSuccess"));
      // onFetchPlace(); // Optional: re-fetch for consistency, or rely on optimistic update
    } catch (error) {
      console.error("Failed to reorder menu items:", error);
      toast.error(t("menuSettings.toast.menuItemsReorderFailedError"));
      onFetchPlace(); // Re-fetch to revert to server state on error
    }
  };

  // Removed duplicate handleOnMenuItemDragEnd function
  ...
  return (
    <MainLayout>
      <div className="d-flex align-items-center mb-4">
        ...
        <h2 className="mb-0 ms-2" style={{ fontWeight: 600 }}>
          {t("menuSettings.title")}
        </h2>
        <div style={{ flexGrow: 1 }} /> {/* Spacer div */}
        <div>
          {" "}
          {/* Wrapper for Dropdown, removed ms-auto */}
          <Dropdown>
            <Dropdown.Toggle
      ...
      <Row>
        <Col md={4}>
          <Panel>
            <h5 style={{ marginBottom: "20px", fontWeight: 600 }}>
              {t("menuSettings.categoriesTitle")}
            </h5>
            <DragDropContext
              onDragStart={() => setIsDragging(true)}
              onDragEnd={handleOnCategoryDragEnd}
            >
                    ...
                    {place.categories && place.categories.length > 0
                      ...
                      : !isDragging && (
                          <p className="text-center text-muted mt-3">
                            {t("menuSettings.noCategoriesYet")}
                          </p>
                        )}
        ...
        <Col md={8}>
          {selectedCategoryId && currentCategory ? (
            <Panel>
              <h5 style={{ marginBottom: "20px", fontWeight: 600 }}>
                {t("menuSettings.menuItemsForCategory", {
                  categoryName: displayCategoryNameForTitle || "",
                })}
              </h5>
              <DragDropContext
                onDragStart={() => setIsDragging(true)}
                onDragEnd={(result) =>
                  handleOnMenuItemDragEnd(result, currentCategory.id)
                }
              >
                        ...
                        ? currentCategory.menu_items.map((item, index) => {
                            let imageUrl = item.image
                              ? item.image.startsWith("http")
                                ? item.image
                                : `${(process.env.REACT_APP_API_URL || "").replace(/\/$/, "")}/${item.image.startsWith("/") ? item.image.substring(1) : item.image}`
                              : null;

                            let itemDisplayLangName = item.name;
                            if (i18n.language === "en" && item.name_en)
                              itemDisplayLangName = item.name_en;
                            else if (i18n.language === "pt" && item.name_pt)
                              itemDisplayLangName = item.name_pt;

                            let itemDisplayLangDesc = item.description;
                            if (i18n.language === "en" && item.description_en)
                              itemDisplayLangDesc = item.description_en;
                            else if (
                              i18n.language === "pt" &&
                              item.description_pt
                            )
                              itemDisplayLangDesc = item.description_pt;
                            const descriptionPreview =
                              itemDisplayLangDesc &&
                              itemDisplayLangDesc.length > 70
                                ? `${itemDisplayLangDesc.substring(0, 70)}...`
                                : itemDisplayLangDesc;

                            return (
                              <Draggable
                                key={String(item.id)}
                                draggableId={`menuitem-${String(item.id)}`}
                                index={index}
                              >
                                {(providedItem, snapshotItem) => (
                                  <StyledListItem
                                    ...
                                    style={{
                                      ...
                                      backgroundColor: snapshotItem.isDragging
                                        ? "#d0eaff"
                                        : "transparent",
                                    }}
                                  >
                                    ...
                                    <div className="actions">
                                      <ActionButton
                                        ...
                                        hovercolor="#007bff"
                                        onClick={() =>
                                          handleShowEditMenuItemModal(item)
                                        }
                                        title={t("common.edit")}
                                      >
                                        <FiEdit2 size={18} />
              ...
              <Button
                ...
                block
                className="mt-3 w-100"
                onClick={handleShowAddMenuItemModal}
              >
                {t("menuSettings.addMenuItem")}
              </Button>
            </Panel>
          ) : (
            <Panel>
              <p className="text-center text-muted">
                {t("menuSettings.selectCategoryPrompt")}
              </p>
            </Panel>
          )}
        </Col>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {t("menuSettings.modal.addNewCategoryTitle")}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
        ...
        <Modal.Footer>
          ...
          >
            {addCategoryLoading
              ? t("common.adding")
              : t("menuSettings.modal.saveCategoryButton")}
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      {editingCategory && (
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              {t("menuSettings.modal.editCategoryTitle", {
          ...
          <Modal.Body>
            <Form.Group className="mb-3">
              <Form.Label>
                {t("menuSettings.modal.categoryNameLabel")} (中文)
              </Form.Label>
              <Form.Control
                type="text"
                placeholder={t(
                  "menuSettings.modal.enterNewCategoryNamePlaceholder",
                )}
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                {t("menuSettings.modal.englishNameLabel")}
              </Form.Label>
              <Form.Control
                type="text"
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                {t("menuSettings.modal.portugueseNameLabel")}
              </Form.Label>
              <Form.Control
                type="text"
                placeholder={t(
                  "editMenuItemForm.placeholder.enterPortugueseName",
                )}
      ...
      {selectedCategoryId && place && (
        <Modal
          ...
          onHide={handleCloseAddMenuItemModal}
          size="lg"
          centered
        >
          <Modal.Header closeButton>
            <Modal.Title>
              {t("menuSettings.modal.addMenuItemTitle", {
                categoryName: displayCategoryNameForTitle || "",
              })}
            </Modal.Title>
          </Modal.Header>
      ...
      {editingMenuItem && place && (
        <Modal
          ...
          onHide={handleCloseEditMenuItemModal}
          size="lg"
          centered
        >
          <Modal.Header closeButton>
            <Modal.Title>
              {t("menuSettings.modal.editMenuItemTitle", {
