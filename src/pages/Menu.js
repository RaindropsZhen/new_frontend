...
import { useParams } from "react-router-dom"; // useLocation removed
...
// const takeawayBoxFee = 8.9; // Removed unused variable
...
const Menu = () => {
  // const { table } = useParams(); // table destructured but params.table used, so removing destructure
  ...
  const [, setShowShoppingCart] = useState(false); // showShoppingCart removed as unused
  ...
  useEffect(() => {
    ...
    const storedTimestamp = localStorage.getItem(
      `orderHistoryTimestamp-${params.table}`,
    );
  ...
  const handleLanguageSelect = (languageLabel) => {
    let langCode = "en"; 
    ...
    localStorage.setItem("selectedLanguage", languageLabel); 
    localStorage.setItem("i18nextLng", langCode); 
  };

  // const location = useLocation(); // isTakeAway below is not used in this component
  // const pathParts = location.pathname.split("/");
  // const isTakeAway = pathParts[pathParts.length - 1].includes("takeaway"); // Removed unused variable
  ...
  const onAddItemtoShoppingCart = (item) => {
    ...
    const numberOfPeople = currentTable ? currentTable.number_people : 1; 
  ...
  const categories =
    place && place.categories
      ? place.categories.map((category) => renderCategoryName(category))
      : [];
  ...
  const OrderHistory = ({ orderHistory }) => {
    ...
    const totalCost = Object.values(groupedItems).reduce(
            ...
            >
              <tr>
                <th style={{ padding: "8px", textAlign: "left" }}>
                  {t("orderHistory.headers.item")}
                </th>
                <th style={{ padding: "8px", textAlign: "right" }}>
                  {t("orderHistory.headers.price")}
                </th>
                <th style={{ padding: "8px", textAlign: "right" }}>
                  {t("orderHistory.headers.quantity")}
                </th>
                <th style={{ padding: "8px", textAlign: "right" }}>
                  {t("orderHistory.headers.total")}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={index} style={{ borderBottom: "1px solid #dee2e6" }}>
                  <td style={{ padding: "8px" }}>
                    {(() => {
                      switch (selectedLanguage) {
                        case "English":
                          return order.name_en || order.name; 
                        case "Português":
                          return order.name_pt || order.name; 
                        case "中文":
                        default:
                          return order.name;
                      }
                  ...
                  <td style={{ padding: "8px", textAlign: "right" }}>
                    €{(order.price * order.quantity).toFixed(1)}
                  </td>
                </tr>
              ))}
              <tr
                style={{ borderTop: "2px solid #dee2e6", fontWeight: "bold" }}
              >
    ...
    <>
      ...
      <Container fluid className="mt-2 mb-4">
        {isLoading ? ( 
          ...
          place.tables &&
          place.tables.find(
            (t) => parseInt(t.table_number) === parseInt(params.table),
          )?.blocked ? (
          <div className="text-center p-4 mt-4 bg-warning border border-warning rounded">
            <h4 className="mb-0">
              This table is currently unavailable. Please contact a staff member
              for assistance.
            </h4>
          </div>
        ) : (
          <>
            ...
            <StickyFilterContainer>
              <Row className="d-flex justify-content-between align-items-center flex-nowrap gap-3">
                ...
                >
                  <option value="">
                    {renderFilterAllButton(selectedLanguage)}
                  </option>
                  {categories.map((name) => (
            ...
            >
              {t("tableNumberDisplayLabel", {
                tableId: params.table === "77" ? "VIP" : params.table,
              })}
            </div>
            ...
            {activeTab === "history" && (
              <OrderHistory activeTab={activeTab} orderHistory={orderHistory} />
            )}
            <BottomTabBar
              activeTab={activeTab}
