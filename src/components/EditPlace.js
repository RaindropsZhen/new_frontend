...
const EditPlace = ({ place, onDone }) => {
  ...
  const [selectedLunchTimeStart, setselectedLunchTimeStart] = useState(
    place.lunch_time_start,
  );
  const [selectedLunchTimeFinish, setselectedLunchTimeFinish] = useState(
    place.lunch_time_end,
  );
  const [selectedDinnerTimeStart, setselectedDinnerTimeStart] = useState(
    place.dinne_time_start,
  );
  const [selectedDinnerTimeFinish, setselectedDinnerTimeFinish] = useState(
    place.dinne_time_end,
  );
  ...
  const onUpdatePlaceInternal = async (formData) => {
    try {
      await updatePlace(place.id, formData, auth.token);
      return true;
    } catch (error) {
  ...
  const handleSubmit = async () => {
    // Renamed onClick to handleSubmit for clarity
    setLoading(true);
    ...
    if (
      selectedLunchTimeStart !== null &&
      typeof selectedLunchTimeStart !== "undefined"
    )
      formData.append("lunch_time_start", selectedLunchTimeStart);
    if (
      selectedLunchTimeFinish !== null &&
      typeof selectedLunchTimeFinish !== "undefined"
    )
      formData.append("lunch_time_end", selectedLunchTimeFinish);
    if (
      selectedDinnerTimeStart !== null &&
      typeof selectedDinnerTimeStart !== "undefined"
    )
      formData.append("dinne_time_start", selectedDinnerTimeStart);
    if (
      selectedDinnerTimeFinish !== null &&
      typeof selectedDinnerTimeFinish !== "undefined"
    )
  ...
  return (
    <div className="page-wrapper">
      <h4 className="text-center mb-4">更新餐厅信息</h4>{" "}
      ...
      <Form.Group className="mb-3">
        {" "}
        {/* Added className for spacing */}
        <Form.Label style={{ fontWeight: "bold", marginRight: "20px" }}>
          输入餐桌数量
        </Form.Label>
