...
const ShoppingCart = ({
  ...
  const [phoneNumber] = useState(0); // setPhoneNumber removed
  const [arrivalTime] = useState(null); // setArrivalTime removed
  const [name] = useState(""); // setName removed
  const [allcomment] = useState({}); // setAllComment removed
  ...
  return (
    <>
      ...
      <h3
        ...
        style={{ fontSize: "clamp(1.2rem, 2vw, 1.8rem)", fontWeight: "bold" }}
      >
