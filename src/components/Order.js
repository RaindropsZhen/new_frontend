  ...
  return (
    <Card className="mb-3 page-wrapper ">
      <Card.Header className="d-flex justify-content-between">
        ...
        <span>{isNew && <span className="new-order">新订单</span>}</span>
        ...
        <div key={order.id} className="printable-section" ref={componentRef}>
          ...
          {JSON.parse(order.detail).map((item) => (
            <div className="mb-2">
              <h3>
                菜名: {item.name} X {item.quantity} -- Prato: {item.name_pt} X{" "}
                {item.quantity}
              </h3>
        ...
        <ReactToPrint
          trigger={() => (
            <button className="small-button" style={printButtonStyle}>
      ...
      <Card.Body className="d-flex justify-content-between">
        <div>
          <span className="print-only">
            {" "}
            {`订单号 #${order.daily_id} - 桌号 ${order.table}`}
          </span>
          {JSON.parse(order.detail).map((item) => (
            <div className="mb-2">
              <img
